...
function registerIPC() {
  ...
  ipcMain.handle('fs:read-project', async (_, filePath) => {
    if (!filePath || path.extname(filePath).toLowerCase() !== '.draco') {
      throw new Error('Only .draco project files may be read via this channel');
    }
  ...
  // Database — raw SQL and bulk secret reads are not exposed to the renderer.
  // Use typed IPC channels only.
  ipcMain.handle('db:get-api-key-status', () => {
    const keys = db?.getApiKeys() || {};
    const status = {};
    for (const [provider, key] of Object.entries(keys)) {
      status[provider] = typeof key === 'string' && key.length > 10;
    }
    return status;
  });
