...
class DatabaseManager {
  ...
  setApiKey(provider, key) {
    ...
    if (!key) {
      ...
      // _encrypt() throws if OS-backed encryption is unavailable — let the error
      // propagate so the UI can inform the user rather than silently dropping the key.
  ...
  _encrypt(str) {
    // Require OS-backed encryption via Electron safeStorage.
    // If unavailable (headless/test/unsupported platform), fail closed rather
    // than falling back to a reversible obfuscation scheme that provides no
    // real protection at rest. Callers must check isEncryptionAvailable() first.
    if (!safeStorage || !safeStorage.isEncryptionAvailable()) {
      throw new Error(
        'Secure storage is unavailable on this system. ' +
        'API keys cannot be persisted until OS-backed encryption is accessible. ' +
        'Try running the app as a normal desktop application.'
      );
    }
  ...
  _decrypt(encoded) {
    if (!encoded) return '';
    // Only safeStorage-encrypted keys (prefixed 'ss:') are supported.
    // Legacy xor-obfuscated values ('xor:' prefix or unprefixed) are no longer
    // decryptable — they were never secure. Return empty string so the user
    // is prompted to re-enter their key rather than silently exposing a secret.
    if (!encoded.startsWith('ss:')) {
      return '';
    }
    if (!safeStorage || !safeStorage.isEncryptionAvailable()) {
      return ''; // Can't decrypt without safeStorage — user must re-enter key
    }
    try {
