...
contextBridge.exposeInMainWorld('draco', {
  ...
  fs: {
    ...
    readProject: (filePath) => ipcRenderer.invoke('fs:read-project', filePath),
  },
  ...
  // ── Database ──
  // Raw SQL (db.query / db.run) and bulk secret reads (db.getApiKeys) are intentionally
  // absent here. All database access must go through typed IPC channels in main.js.
  db: {
    getApiKeyStatus: () => ipcRenderer.invoke('db:get-api-key-status'),
