...
type RankingScope = 'all' | 'identity' | 'uncaptioned' | 'custom'
...
function SessionSetupModal({ onClose, onCreated, projectId }: SessionSetupModalProps) {
          ...
          <div className="flex gap-2">
            {(
              [
                ...
                { value: 'identity', label: 'By Identity' },
                { value: 'uncaptioned', label: 'Uncaptioned Only' },
            ...
            {(
              [
                ...
                },
                {
                  value: 'balanced',
                  label: 'Balanced',
                  desc: 'Mix of uncertainty and random',
                },
              ] as const
            ).map((opt) => (
              <label
...
function ArenaTab({ projectId }: ArenaTabProps) {
  ...
  const {
    ...
    queryKey: ['ranking-pair', selectedSessionId],
    queryFn: () => rankingApi.getNextPair(selectedSessionId!, activeSession?.ranking_algorithm === 'elo' ? 'random' : 'uncertainty'),
  ...
  }, [currentPair, handlePick, handleDraw, handleSkip, compareMutation.isPending, pairFetching])

  const isWorking = compareMutation.isPending || pairFetching || pairLoading
                ...
                {leaderboard.map((entry) => {
                  ...
                  return (
                    <tr
                      ...
                      className={clsx(
                        'border-b border-border/50 cursor-pointer transition-colors hover:bg-surface-elevated',
                        isSelected && 'bg-accent/10'
                      )}
                    >
                      <td className={clsx('px-3 py-2 font-bold tabular-nums text-sm', rankColor)}>
                        {entry.rank}
                      </td>
                      ...
                      <td className={clsx('px-3 py-2 text-xs truncate max-w-[200px]', rankColor)}>
                        {entry.filename}
                      </td>
                      <td className={clsx('px-3 py-2 text-right font-mono font-medium tabular-nums', rankColor)}>
                        {entry.mu.toFixed(2)}
                      </td>
