...
@pytest.mark.asyncio
async def test_augmentation_preview_endpoint_serves_result_file(client, db):
    from config import settings
    project = await _create_project(db, "preview")
    asset = await _create_asset(db, project.id, "source.png")
    # Write the preview file inside managed storage so the security check passes
    preview_dir = Path(settings.DATA_DIR).resolve()
    preview_dir.mkdir(parents=True, exist_ok=True)
    output = preview_dir / "preview_test.png"
...
async def test_augmentation_preview_endpoint_404s_for_missing_file(client, db):
    ...
    asset = await _create_asset(db, project.id, "source.png")
    # Use a path inside managed storage that does not actually exist
    missing_path = Path(settings.DATA_DIR).resolve() / "augmentation" / "definitely_missing_preview.png"
    job = AugmentationJob(
        project_id=project.id,
        source_asset_id=asset.id,
        augmentation_type="outpaint",
