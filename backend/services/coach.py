...
class DatasetCoach:
    ...
    async def analyze(
        ...
        return CoachReport(
            ...
            keep_first=self._select_keep_first(assets),
    ...
    def _select_keep_first(self, assets: list[Asset]) -> list[str]:
        # Top 10 by composite_score, excluding rejected
        eligible = [a for a in assets if not a.is_rejected]
        sorted_assets = sorted(
            eligible,
            key=lambda a: a.composite_score or 0.0,
            reverse=True,
        )
        return [a.id for a in sorted_assets[:10]]
